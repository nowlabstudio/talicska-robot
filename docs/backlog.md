...
- **Foxglove Restart gomb — `/robot/restart` topic + `restart_watchdog`** — 2026-03-24. Foxglove layout-ban Publish panel: `/robot/restart` (std_msgs/Bool, `{data: true}`). HOST-oldali watchdog: `scripts/restart_watchdog.sh` + `scripts/systemd/talicska-restart-watchdog.service` (Restart=always). A watchdog `docker compose exec -T robot ros2 topic echo /robot/restart --count 1` blokkoló subscribe-bal figyel, majd `systemctl restart talicska-robot.service`-t hív. **TELEPÍTÉS szükséges:** `bash scripts/install.sh` futtatása a `talicska-restart-watchdog.service` regisztrálásához. Érintett: `scripts/restart_watchdog.sh`, `scripts/systemd/talicska-restart-watchdog.service`, `scripts/install.sh`.
