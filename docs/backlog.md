...
- **~~RoboClaw TCP disconnect nem detektált — robot 4s-ig fékezés nélkül halad (KRITIKUS)~~** — ✅ **KÉSZ (2026-03-22):** Két rétegű detektálás implementálva. (1) **`roboclaw_hardware`** plugin: `/hardware/roboclaw/connected` topic most **10 Hz heartbeat**-et publikál (korábban csak change-on) + azonnali `false` TCP kieséskor. (2) **`safety_supervisor`**: `roboclaw_status_timeout_s` (default 0.3 s) csend-watchdog — ha a driver crashel/fagy és abbahagyja a publikálást, 3 kihagyott heartbeat után `joint_states_dropout_latch_ → FAULT`. Log megmarad (`RCLCPP_ERROR`, latch nem törlődik automatikusan). Reset: E-Stop press+release (ha TCP reconnected) VAGY `/robot/reset`. `roboclaw_status_timeout_s: 0.3` a `robot_params.yaml`-ban. Érintett: `ROS2_RoboClaw/src/roboclaw_hardware.cpp`, `ROS2_RoboClaw/include/roboclaw_hardware/roboclaw_hardware.hpp`, `robot_safety/src/safety_supervisor.cpp`, `config/robot_params.yaml`.
