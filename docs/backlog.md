...
- **🔴 Foxglove 4-6s lag a T580 (Win laptop WiFi-n) — NYITOTT, holnap folytatandó (2026-05-10)** — Tünet: a T580-on minden Foxglove topic 4-6s késéssel jelenik meg, robot URDF be sem töltődik. A 109 (Mac mini Ethernet) ugyanazon a robot stacken **villámgyors**. Felhasználó: "tegnap minden ment realtime, kamera+pointcloud is" — tehát konkrét konfig változás visszafordítható.

  **Kontextusban változott (felhasználó utalása):** Tailscale + Portainer telepítés, ezzel együtt enP8p1s0 + WiFi konfig módosítás. **Ami a sessionben perzisztens változott:** `nmcli` enP8p1s0 +ipv4.routes metric 100, `cyclonedds.xml` MaxAutoParticipantIndex 32→64. **Ami a sessionben runtime változott (NEM perzisztens, holnap dönteni kell):** strict ARP mód (announce=2, ignore=1, filter=1) az enP8p1s0 + wlx interfészeken.

  **Eddig kizárt hipotézisek (NO-GO):** stale bind mount, MaxAutoParticipantIndex exhaustion, kliens browser CPU/render, WebSocket backpressure (`send_buffer_limit:2MB`), TCP konfig (MTU/retrans/rwnd OK), Tailscale iptables, `ip_forward=1`, kétkliens WiFi-contention (felhasználó elutasította).

  **Részleges hatású fix-ek (GO részben):** aszimmetrikus routing fix → 109-et megoldotta, T580-at NEM. Strict ARP → `lastrcv` 36s→5s, wlx duplikáció 52→21 KB/5s, **DE 4-6s lag MARAD**. Jetson WiFi disconnect 20s → RTT 49→38ms, lag változatlan.

  **Jellemző mérési értékek a T580-ra:** Send-Q 3-4 MB, RTT 45-96 ms, retrans 0.25-0.37%, rwnd_limited 24-30%, `delivery_rate` 123 Mbps (network bír), `lastrcv` 5-36s (TCP keepalive nem ejti). 109-en ugyanezek: Send-Q 0, RTT 1.3 ms, retrans 0.04%, rwnd_limited 0%.

  **GO irányok holnapra (még nem próbáltunk):**
  1. **Tegnapi git állapot konkrét vissza-tesztelés** — `git checkout 815dee8` (a 7c96e64 előtti commit), majd `docker compose restart` minden konténer. Ha "tegnapi" konfig esetén a T580 is gyors → a `nmcli +ipv4.routes 100` vagy a `cyclonedds.xml` 32→64 emelése **valamiért rontotta** a T580 helyzetét. Ha nem javul → a változás **kívül** van a git-en (sysctl, NetworkManager profile, host config).
  2. **iperf3 valós throughput mérés** T580 ↔ Jetson 192.168.68.124 (WiFi) **és** 192.168.68.200 (Eth) IP-n. Ha 124-en alacsony, 200-on magas → multi-homed routing szűkítő. Ha mindkettőn alacsony → T580 oldali WiFi/hardware. Ha mindkettőn magas → alkalmazás-szintű probléma.
  3. **Foxglove URL audit** a T580 oldalán (browser cache, layout fájl) — explicit `ws://192.168.68.200:8765` LAN IP, NEM `synapse.tailbbafca.ts.net:8765` (Tailscale magic DNS), NEM `100.116.200.82:8765` (Tailscale IP). Tailscale tunnel a T580-on `active; direct 192.168.68.125:41641, tx 7.9 GB rx 261 MB` — gyanús hogy a tegnapi forgalom Tailscale-en ment, és a kliens cache-ben még a magic DNS van.
  4. **Persistence dilemma a runtime sysctl-ekre** — strict ARP mód reboot után visszamegy default-ra. Ha a 109-re vonatkozó pozitív hatás (lastrcv 36→5s, wlx duplikáció -60%) tartós kell, `/etc/sysctl.d/99-talicska-arp.conf` készítendő.
  5. **CycloneDDS `lo+wlx` → `lo-only` revízió** — cross-host DDS nincs (lásd külön backlog tétel), wlx-ben hirdetett locator felesleges és potenciálisan zavaró.
  6. **Eth + WiFi UGYANAZON /24-en** strukturális megoldása: a wlx-et másik subnet-re vagy másik AP-ra (5GHz dedikált), hogy ne legyen multi-homed konfliktus. Vagy: explicit `ipv4.never-default yes` és külön "rule" routing tábla.

  **NO-GO irányok (NE PRÓBÁLD újra):** force-recreate konténerek (már megvolt), MaxAutoParticipantIndex emelés (már 64), `ip_forward=0` lekapcsolás (nincs hatása), Foxglove layout szűkítés mint elsődleges fix (a 109 layout nagyméretű és gyors, T580 layout szűkítve sem volt jó), WiFi sávszélesség szelet hipotézis önmagában (Eth IP-n is van lag a T580-on a 200-on csatlakozva).
