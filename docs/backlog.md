...
- **Robot leállításkor `startup_supervisor` OFF állapotba küldése** — `make down` során a startup_supervisor-nak tudnia kellene, hogy szándékos leállás történik (nem crash). Jelenleg a node csak a container leállásával szűnik meg, de `/startup/state` nem kerül `OFF` vagy `SHUTDOWN` állapotba — Foxglove-ban az utolsó látott állapot megmarad. Megoldás: `make down`-ban `ros2 topic pub --once /robot/shutdown std_msgs/msg/Bool '{data: true}'` VAGY `ros2 param set /startup_supervisor shutdown true` a `docker compose stop` előtt, és a startup_supervisor kezelje ezt az állapotot. Érintett: `robot_safety/src/startup_supervisor.cpp`, `Makefile`.
