...
- **RoboClaw TCP disconnect nem detektált — robot 4s-ig fékezés nélkül halad (KRITIKUS)** — 2026-03-21, mérve. Ha a RoboClaw RJ45 kihúzva mozgás közben: a safety_supervisor nem érzékeli, állapot IDLE marad. A robot a RoboClaw belső watchdog timeout-ig (~4s) teljes sebességgel halad, csak utána áll meg. **100kg robot 14 km/h közelében ez elfogadhatatlan.** A startup_supervisor precheck észleli a hiányzó motor drivert és megállítja a bootot — de futás közben nincs védelem. Fix opciók: (1) `/joint_states` topic watchdog a safety_supervisorban — ha a controller stack meghal (RoboClaw TCP drop → hw interface exception), a `/joint_states` publish megáll; (2) RoboClaw hardware interface publishol `/robot/hw_ok` (Bool) heartbeat topicot, safety_supervisor watchdog-ja figyeli; (3) RoboClaw hardware interface FAULT state-et diagnosztika topicon átvezetni. Preferált megoldás: (1) — meglévő topic, nincs firmware változás. Érintett: `robot_safety/src/safety_supervisor.cpp`, esetleg `roboclaw_hardware`.
