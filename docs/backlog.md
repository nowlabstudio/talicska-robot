...
- **RoboClaw TCP disconnect nem detektált a safety rétegben** — 2026-03-21. Ha a RoboClaw TCP kapcsolat leesik (USR-K6 kábel kihúzva), a hardware interface szinten kezelve van (reconnect logika), de a `safety_supervisor` nem látja az eseményt. Robot módban: a `diff_drive_controller` leállhat, a robot megáll — ez véletlenszerűen biztonságos, de nem garantált. Fix opcióit vizsgálni: (1) `/joint_states` topic watchdog — ha a controller stack leáll, ez is megszűnik; (2) hardware interface publishol egy `/robot/hw_ok` Bool topicot ami a safety_supervisor watchdog inputja lesz; (3) RoboClaw hardware interface FAULT stateét diagnosztika topicon átvezetni a safety_supervisorba. Érintett: `roboclaw_hardware`, `robot_safety/src/safety_supervisor.cpp`.

- **RealSense watchdog — enable_imu_watchdog engedélyezendő** — 2026-03-21. RealSense kihúzva: `safety_supervisor` nem detektálja, mert `enable_imu_watchdog: false` és `tilt_roll_limit_deg: 90` (watchdog inaktív). Ha az IMU watchdog be lenne kapcsolva, a `/camera/camera/imu` topic timeout → `imu_dropout_latch_` → ERROR state. Előfeltétel az engedélyezéshez: IMU frame orientáció korrekció (tilt check IMU tilt backlog item), utána `enable_imu_watchdog: true` beállítható `robot_params.yaml`-ban. Addig: a RealSense kiesése nem látszik. Érintett: `config/robot_params.yaml`.
